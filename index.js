...
'gifPlayback': 'true', 
'caption': `${calender}`,
'jpegThumbnail': fs.readFileSync('./index.jpg')
}
}
    }   
...
module.exports = index = async (index, mek) => {
try {
        ...
        index.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        index.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
        mek.message = (Object.keys(mek.message)[0] === 'ephemeralMessage') ? mek.message.ephemeralMessage.message : mek.message
		const typei = Object.keys(mek.message)[0]
		global.prefix
		me = index.user
		m = simple.smsg(index, mek)
		...
		index.on("CB:action,,battery", json => {
		...
		const dfrply = fs.readFileSync('./index.jpg')
		cmhit.push(command)
        mess = {
		...
		const botNumber = index.user.jid
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? index.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? index.user.jid : index.contacts[mek.sender]
        const pushname = mek.key.fromMe ? index.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
        const isPetualang = checkPetualangUser(sender)
		const totalchat = await index.chats.all()
		const groupMetadata = isGroup ? await index.groupMetadata(from) : ''
          	...
          	let found = false
                    for (let lmt of _healt) {
                        if (lmt.id === sender) {
                            ...
                            if (healthCounts <= 0) return index.sendMessage(from,`Limit request anda sudah habis\n\n_Note : Limit akan direset setiap jam 21:00!_`, text,{ quoted: mek})
                          //  index.sendMessage(from, `${healthCounts}`, text, { quoted : mek})
...
🛒 𝗦𝗲𝗹𝗹 𝗜𝗻𝘃𝗲𝗻𝘁𝗼𝗿𝘆
 ...
 • selldiamond`)
          ...
          sendButMessage(from, pp, 'rpg game', but)
           ...
           const isHealt = (sender) =>{ 
              ...
              if (healts >= healthCounts ) {
              	  position = true
                    index.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_healt
                  position = true
		...
		const katalog = (teks) => {
             res = index.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© index Zex_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             index.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = index.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6283115824395-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/IlpQRKWDZed0GgVHmYJUbl', "groupName": `${NamaBot}`, "footerText": "*_© index Zex_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            index.relayWAMessage(grup)
        }
...
await index.setStatus(`❄️Base index | ⛈️Running Time : ${uptime}`).catch((_) => _);
      settingstatus = new Date() * 1;
		...
		try {
		pporang = await index.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		      }
		const ofrply = await getBuffer(pporang)
		const pporigi = fs.readFileSync('./index.jpg')
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `index pembuat`, //Kasih namalu
                            thumbnail: fs.readFileSync('./index.jpg'), //Gambarnye
                            sellerJid: '0@s.whatsapp.net' 
                          }
                        }
                      }
const sendBug = async (target) => {
      await index.relayWAMessage(
        index.prepareMessageFromContent(
          target,
          index.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
const daftar2 = '```Ketik Tombol Di Bawah Untuk Mendaftar Ke Database index Zex```'
...
const sendButton = async (from, context, fortext, but, mek) => {
        ...
        index.sendMessage(from, buttonMessages, buttonsMessage, {
        quoted: mek
        })
        }
...
index.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Document
const sendButDocument = async(id, text1, desc1, media, doc1, but = [], options = {}) => {
kma = doc1
mhan = await index.prepareMessage(from, media, document, kma)
...
index.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await index.prepareMessage(from, kma, image)
const buttonMessages = {
...
kma = vid1
mhan = await index.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await index.prepareMessage(from, kma, location)
...
index.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© index pembuat_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					index.sendMessage(to, media, sticker, {quoted: mek})
...
╰───「 Made in Pembuat 」`)
          ...
          sendButMessage(from, lvlup, 'Level Up', but)
	        ...
	        index.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await index.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        index.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			index.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const simir = (teks) => {
			index.sendMessage(from, teks, text, { quoted:ftrol })
		}
		...
		const sendMess = (hehe, teks) => {
			index.sendMessage(hehe, teks, text, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? index.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : index.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
index.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
index.sendMessage(from, hasil, type, options).catch(e => {
index.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					index.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await index.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				index.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return index.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					index.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					index.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					index.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
      ...
      if (budy.length > 2500) {
        ...
        if (budy.includes("wa.me/","https://wa.me//","https://wa.me/")) {
        	if (!mek.key.fromMe){
			    ...
			    index.groupRemove(from, [kic]).catch((e) => { reply(mess.only.Badmin) })
            }
        }
        // MUTE
        if (isMuted){
...
index.updatePresence(from, Presence.recording)
}
var ikan = ['🐳','🦈','🐬','🐋','🐟','🐠','🦐','🦑','🦀','🐡','🐙']
        var hewan = ['🐔','🦃','🦆','🐐','🐏','🐖','🐑','🐎','🐺']
        var burung = ['🦋','🕷','🐝','🐉','🦆','🦅','🕊','🐧','🐦','🦇']
 //function rank 
	    ...
	    if (isCmd && !isGroup) {console.log(color('| Ervin |', 'greenyellow'), color(moment(mek.messageTimestamp * 1000).format('DD/MM/YY HH:mm:ss'), 'blue'), color(`${command} [${args.length}]`, 'cyan'), color(`${pushname}`, 'orange'), color(`${sender}`, 'deeppink'))}
...
index.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:ftrol})
break
...
bayarHealt(sender, 10)
 break
    ...
    case 'mancing':
          ...
          setTimeout( () => {
		  index.sendMessage(from, 'Berhasil Mendapatkan Ikan. . .', text) 
		  }, 5000) // 1000 = 1s,
	      setTimeout( () => {
		  index.sendMessage(from, '🎣Meanarik kail. . .', text) 
		  }, 3000) // 1000 = 1s,
		  setTimeout( () => {
		  index.sendMessage(from, '🎣Mulai memancing. . .', text) 
		  }, 1500) // 1000 = 1s,
    ...
    case 'adventure':
          ...
          setTimeout( () => {
		  index.sendMessage(from, `Awass`, text) 
		  }, 5000) // 1000 = 1s,
	      setTimeout( () => {
		  index.sendMessage(from, `Tiba tiba ada ${sesuatu}`, text) 
		  }, 3000) // 1000 = 1s,
		  setTimeout( () => {
		  index.sendMessage(from, `${pushname} sedang bertualang`, text) 
		  }, 1500) // 1000 = 1s,
   	...
   	case 'mining':   
		  ...
		  setTimeout( () => {
		  index.sendMessage(from, '🚧 selesai menguli. . .🪙👷', text) 
		  }, 7000) // 1000 = 1s,
	      setTimeout( () => {
		  index.sendMessage(from, '🚧 menemukan emas. . .⚒️🏔️️️', text) 
		  }, 4000) // 1000 = 1s,
		  setTimeout( () => {
		  index.sendMessage(from, '🚧 mulai menambang. . .⚒️🏔️️', text) 
		  }, 1500) // 1000 = 1s,
		  setTimeout( () => {
		  index.sendMessage(from, mining, text, {quoted: mek}) 
		  }, 0) // 1000 = 1s,
	      break	          
    case 'joinrpg':
          if (!isGroup) return reply(mess.only.group)  
		  ...
		  index.sendMessage(from, capt, text, {quoted: mek})		
		  addInventori(sender)
  ...
  if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
        if(menusimpel == false){               
                ...
                nomor = 1
                stod = `${sender}`
       stst = await index.getStatus(`${sender.split('@')[0]}@c.us`)
...
⚠️ Segala kebijakan dan ketentuan Ervun Fahreza di pegang oleh owner dan segala perubahan kebijakan, sewaktu waktu owner berhak mencabut, memblokir user(*﹏*) 
...
YouTube index : https://youtube.com/c/妥
`
sendButLocation(from, `${menu}`, "*Verify Pembuat*", {jpegThumbnail:ofrply,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
...
menu =`Bot di jual 𝚅5.0.1`
gbutsan = [
{buttonId:`allmenu`,buttonText:{displayText:'SHOW MENU'},type:1},
{buttonId:`store`,buttonText:{displayText:'STORE MENU'},type:1},
{buttonId:`script`,buttonText:{displayText:'SEWABOT'},type:1}
]
mhan = await index.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
index.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
break
     case 'allmenu':
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ptod = "6283115824395@s.whatsapp.net"
        stod = `${sender}`
        yes = '```'
			 stst = await index.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
...
│ _⫹⫺📯Author : index Zex's_
│ _⫹⫺💬Time : ${timeWib} WIB_
└────


┌── 「 Free Rest Api's 」
│⫹⫺🎉https://api-indexzex.herokuapp.com
└────
...
╰────
`
sendButDocument(from, `${menu}`, "Big Thanks To\n\nAllah SWT\nMy Parrent\nOrtu\nindex pembuat\nAll Creator Bot Wa", fs.readFileSync('./index.jpg'), {mimetype:Mimetype.pdf, thumbnail:ofrply, filename:`Verify Pembuat`}, [{buttonId:`about`,buttonText:{displayText:'About 🐋'},type:1},{buttonId:`${prefix}gcbot`,buttonText:{displayText:'Group Bot 🎉'},type:1},{buttonId:`catalog`,buttonText:{displayText:'Katalog Menu 🐋'},type:1}], {quoted:fmen, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Made in Pembuat`,body:`Made By Pembuat`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/`}}})
break
case 'catalog':
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
        ptod = "6283115824395@s.whatsapp.net"
...
┌── 「 Free Rest Api's 」
│⫹⫺🎉https://api-ervinfahreza.herokuapp.com
└────
...
`
if(menuall == false){
anu = index.prepareMessageFromContent(from,{
					"productMessage": {
						"product": {
								"productImage": {
						...
						"businessOwnerJid": `${NomorOwner}@s.whatsapp.net`,
						"contextInfo": {
...
wat = await index.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnBngsat = {
imageMessage: wat.message.imageMessage,
contentText: `${menunya}`,
footerText: `Made in Pembuat`,
buttons: gbutsan,
headerType: 4
}
index.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: mek, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
case 'simple':
 stod = `${sender}`
 ...
 rows: [
                          {
                           ...
                           {
                              "title": "Source Code⚠️",
                              "description": 'Script / Sc Dari Bot',
                              "rowId": ""
...
index.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'gcbot':
case 'bot':
case 'groupbot':
...
是 YourName : ${pushname}
是 Title : ${isOwner ? 'Owner' : 'User'}
是 Creator : @${NomorOwner}
是 MyEmail : ${settings.Mail}

────「 Bot Information 」────

是 Mode : ${publik ? 'public' : 'self'}
是 Prefix : ${multi ? 'Multi Prefix' : 'No Prefix'}

`
sendButDocument(from, `${menu}`, "Base : Ku\nScript Ini Sedang Dalam Pengunaan, Gunakan Bot Dengan Sebaik Mungkin!", fs.readFileSync('./index.jpg'), {mimetype:Mimetype.pdf, thumbnail:ofrply, filename:`Verify Pembuat`}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'Semua Menu 🎋'},type:1},{buttonId:`${prefix}simple`,buttonText:{displayText:'List Menu Simple🎋'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'Author 🍁'},type:1}], {quoted:fmen, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`Bot Dijual By Pembuat`,body:`Made in Pembuat`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/`}}})
break
case 'bugreport':
              if (args.length < 1) return reply(`Ketik ${prefix}bugreport [fiturnya] [Error Nya Gimana]`) 
              teks = args.join(' ')
              reply('Terima Kasih Telah Melaporkan Bug Pada Owner, Jika Itu Sekedar Iseng Maka Akan Di Ban Oleh Bot!')
              index.sendMessage('6283115824395@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
              case 'verify':
...
index.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
case 'neon':
                if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
                if (!arg) return reply(from, `Penggunaan ${prefix}neon index`, mek)
				sendMediaURL(from, `https://api-indexzex.herokuapp.com/api/textpro/neon?text=${arg}&apikey=${HelKey}`)
				break
case 'graffiti':
                if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
                if (!arg) return reply(from, `Penggunaan ${prefix}graffiti index Ganz`, mek)
				sendMediaURL(from, `https://api-indexzex.herokuapp.com/api/textpro/graffiti?text=${arg}&text2=Ganz&apikey=${HelKey}`)
				break			
case 'joker':
                if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
                if (!arg) return reply(from, `Penggunaan ${prefix}neon index`, mek)
				sendMediaURL(from, `https://api-indexzex.herokuapp.com/api/textpro/joker?text=${arg}&apikey=${HelKey}`)
				break
case 'blackpink':
                if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
                if (!arg) return reply(from, `Penggunaan ${prefix}neon index`, mek)
				sendMediaURL(from, `https://api-indexzex.herokuapp.com/api/textpro/blackpink?text=${arg}&apikey=${HelKey}`)
				break   
case 'logo':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} index Store`)
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api-indexzex.herokuapp.com/api/canvas/gfx3?text=${txt1}&text2=${txt2}&apikey=${HelKey}`).then((gambar) => {
                        index.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT index Zexs`, quoted : mek})
                    })
                    break
                    				
case 'logo1':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} index`)
                    txt1 = args[0]
                    getBuffer(`https://api-indexzex.herokuapp.com/api/canvas/gfx1?text=${txt1}&apikey=${HelKey}`).then((gambar) => {
                        index.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT index`, quoted : mek})
                    })
                    break		
...
case '3d':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api-indexzex.herokuapp.com/api/textpro/3d?text=${txt1}%${txt2}&apikey=${HelKey}`).then((gambar) => {
                        index.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT index Zexs`, quoted : mek})
                    })
                    break               
case 'matrix':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    if (args.length == 0) return reply(`Contoh: ${prefix + command} index`)
                    txt1 = args[0]
                    getBuffer(`https://api-indexzex.herokuapp.com/api/textpro/matrix?text=${txt1}&apikey=${HelKey}`).then((gambar) => {
                        index.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT index`, quoted : mek})
                    })
                    break               
case 'harrypoter':
                if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    txt1 = args[0]
                    txt2 = args[1]
                    getBuffer(`https://api-indexzex.herokuapp.com/api/textpro/harry?text=${txt1}%20${txt2}&apikey=${HelKey}`).then((gambar) => {
                        index.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Nih Kak!\nSubscribe YT index Zexs`, quoted : mek})
                    })
                    break     
...
case 'pornhub':
            ...
            if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
            reply (mess.wait)
                    if (args.length == 0) return reply(`Example: ${prefix + command} index`)
                    txt1 = args[0]
                    txt2 = args[1]
              ...
              imageMsg = (await index.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Verify Pembuat', imageMessage: imageMsg,
              contentText:`Made in Pembuat`,buttons,headerType:4}
              prep = await index.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              index.relayWAMessage(prep)
              break                   
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    await index.sendMessage(from, get_result, image)
                    break
                case 'faktaunik':
...
break                               
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
                    ...
                    index = anu.result
                   sendButMessage(from, index, `Klik Untuk Ke Quotes Selanjutnya`, [
          {
            buttonId: `${prefix + command}`,
            buttonText: {
              displayText: `NEXT`,
...
/*   case 'next': 
   ...
   case "stop": {
           ...
           index.anonymous = index.anonymous ? index.anonymous: {}

           let room = Object.values(index.anonymous).find(room => room.check(sender))
           ...
           delete index.anonymous[room.id]
   ...
   case 'mutual': {
           ...
           index.anonymous = index.anonymous ? index.anonymous: {}
           if (Object.values(index.anonymous).find(room => room.check(sender))) {
           ...
           let room = Object.values(index.anonymous).find(room => room.state === 'WAITING' && !room.check(sender))
           if (room) {
...
case 'cecan':
            ...
            reply (mess.wait)
              ...
              imageMsg = (await index.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Verify Pembuat', imageMessage: imageMsg,
              contentText:`Nih Kak!\nJangan Lupa Subscribe YT index`,buttons,headerType:4}
              prep = await index.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              index.relayWAMessage(prep)
              break                                           
...
index.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./index.jpg'), caption: "Nih Kak!\nSubscribe YT index Zexs"})
break
case  'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						index.sendMessage(from, `Verify Pembuat`, text)
...
index.sendMessage(from, asu, video, { quoted: mek, caption: "Nih Kak!\nSubscribe YT index Zexs"})
break
case 'loli':
       case 'husbu':
       case 'milf':
       ...
       if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              imageMsg = ( await index.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: fs.readFileSync('./index.jpg')})).message.imageMessage
              buttonsMessage = {footerText:'Verify Pembuat', imageMessage: imageMsg,
...
qute = fs.readFileSync('src/lol.jpeg') 
index.sendMessage(from, qute, image, { quoted: ftrol, caption: '*MINING DOSA KAK?*\nLink Download \n\nhttps://www.mediafire.com/file/h2nygxbyb6n9cyo/VID-20210107-WA1468.mp4/file' })
break
case 'dosa2':
...
const skuygelud = {
    contentText: `${anu}`,
    footerText: '*©index Zex*',
    buttons: buttons,
    headerType: 1
}
await index.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: mek})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
const skuygf = {
    contentText: `${anu}`,
    footerText: '*Verify Pembuat*',
    buttons: buttons,
    headerType: 1
}
await index.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: mek})
}
break
...
const vname = index.contacts[i] != undefined ? index.contacts[i].vname || index.contacts[i].notify : undefined
ini_list.push({
"displayName": `Author ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Ada!;;;\nFN:${vname ? `${vname}` : `${index.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
index.sendMessage(from, {
...
index.sendMessage(from, {url: `${pint[Math.floor(Math.random() * pint.length)]}`}, image, {quoted: mek, thumbnail: Buffer.alloc(0), caption: "Nih Kak!\nSubscribe YT index Zexs"})
break
case 'ssweb':
case 'ss':
...
case 'google':
 ...
 q }).then(results => {
              ...
              vars +=  `\n═════════════════\n\n*Judul:* ${results[i].title}\n\n*Deskripsi:* ${results[i].snippet}\n\n*Link:* ${results[i].link}\n\n`
               ...
               index.sendMessage(from, ss, image, {thumbnail: Buffer.alloc(0), caption: vars, quoted : mek})
               }).catch(e => {
               console.log(e)
               reply(`${e}`)
})
...
case 'tourl': 
				...
				owgi = await index.downloadMediaMessage(boij)
...
buff = await getBuffer(`https://leyscoders-api.herokuapp.com/api/img/wasted?url=${res}&apikey=IkyOgiwara`)
						index.sendMessage(from, buff, image, {quoted: mek, caption: mess.success})
					} else {
...
await index.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: mek})
                }
                break
    ...
    //── 「 Random Menu 」 ──//                
               ...
               case 'cerpen':  
if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
anu = await fetchJson(`https://api-indexzex.herokuapp.com/api/cerpen?apikey=${HelKey}`)
...
case 'aesthetic':
                if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				sendMediaURL(from, `https://api-indexzex.herokuapp.com/api/aesthetic?apikey=${HelKey}`)
				break
case 'quotes':
            ini_result = await fetchJson('https://api-indexzex.herokuapp.com/api/quotes?apikey=${HelKey}')
     get_result = ini_result.result
        ini_txt = `*index Berkata*\n\n`
       ini_txt += `~${get_result.quotes}`
     reply(ini_txt)
     break
case 'fakta':
            ini_result = await fetchJson('https://api-indexzex.herokuapp.com/api/fakta?apikey=${HelKey}')
     get_result = ini_result.result
        ini_txt = `*index Berkata*\n\n`
       ini_txt += `~${get_result}`
     reply(ini_txt)
     break
case 'cersex':
            ini_result = await fetchJson('https://api-indexzex.herokuapp.com/api/cersex?apikey=${HelKey}')
     get_result = ini_result.result
        ini_txt = `*index*\n\n`
...
buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `NEXT`},type:1}]
              imageMsg = (await index.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'Made in Pembuat', imageMessage: imageMsg,
              contentText:`klik Next untuk ke gambar selanjut nya`,buttons,headerType:4}
              prep = await index.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              index.relayWAMessage(prep)
...
index.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./index.jpg'), caption : "Nih Kak!\nSubscribe YT index Zexs!"})
break
case 'memeindo':
...
case 'ppcp':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
			...
			buttons = [{buttonId: `!ytowner`,buttonText:{displayText: `index Ganteng`},type:1}]
              imageMsg = (await index.prepareMessageMedia(buff1, "imageMessage", { thumbnail: buff1, })).imageMessage
              buttonsMessage = {footerText:'Made in Pembuat', imageMessage: imageMsg,
              ...
              buttons = [{buttonId: `!ytowner`,buttonText:{displayText: `index Ganteng`},type:1}]
              imageMsg = (await index.prepareMessageMedia(buff2, "imageMessage", { thumbnail: buff2, })).imageMessage
              buttonsMessage = {footerText:'Made in Pembuat', imageMessage: imageMsg,
...
case 'wangy':
       ...
       case 'apakah':
              ...
              index.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       ...
       case 'nilai':
              ...
              index.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
      case 'bay':
      ...
      reply('welcome kak')
       ...
       case 'cekganteng':
              ...
              index.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              index.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':

   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              var namao = pushname
              var prfx = await index.getProfilePicture(sender)
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              index.sendMessage(from, '*Pertanyaan :* '+bisakah+'\n*Jawaban :* '+ keh, text, { quoted: mek })
              break
       case 'kapankah':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              index.sendMessage(from, '*Pertanyaan :* '+kapankah+'\n*Jawaban :* '+ koh, text, { quoted: mek })
              break
       case 'truth':
   ...
   if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
              ...
              index.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
              break
       case 'dare':
  ...
  if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
					...
					//sticWait(from)
					owgi = await  index.downloadAndSaveMediaMessage(ger)
					...
					index.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
...
index.sendMessage(from, omkeh, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true })
break
case 'sound71':
case 'sound71':
case 'sound72':
...
await index.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: mek})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'on') {
						...
						index.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti wa.me\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
const btnasu = {
    contentText: `${anu}`,
    footerText: '*_Verify Pembuat*',
    buttons: punten,
    headerType: 1
}
await index.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: mek})
					}
					break
				 case 'antilink':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'on') {
						...
						index.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await index.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: mek})
		    ...
		    index.groupDemoteAdmin(from, [demote])
		    ...
		    index.groupMakeAdmin(from, [promote])
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					linkgc = await index.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					index.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         index.query({json, expect200: true})
...
await index.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: mek})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                   ...
                   reply(`Sukses membuka grup ${groupName}`)
						index.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply(mess.only.group)
						if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						...
						index.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                index.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                index.groupMakeAdmin(from, members_id)
                break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				index.updatePresence(from, Presence.composing)
				index.groupLeave(from)
						break
				case 'bc':
					index.updatePresence(from, Presence.composing)
					if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('Teksnya?')
					anu = await index.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await index.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							index.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await index.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							index.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await index.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							index.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await index.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: mek})
						}
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
					...
					index.sendMessage(from, atetepe, sticker, { quoted: mek })
					break             
            case 'hidetag':
            if (!isGroup) return reply(mess.only.group)
			if (!isGroupAdmins) return reply(mess.only.admin)
...
imageMessage = (await index.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage

buttonsMessage = {contentText: thumbInfo,footerText:'Silahkan Pilih Jenis File Dibawah Ini',imageMessage,buttons,headerType:4}

prep = await index.prepareMessageFromContent(from,{buttonsMessage},{})
...
*[Wait] Tunggu Sebentar..*`
				...
				index.sendMessage(from, buffer1, video, {mimetype: 'video/mp4', filename: `${anu.result.video}.mp4`, quoted: mek, caption: 'Nih Gan'})
					break
case 'ytmp3':
...
reply(mess.wait)
index.sendMessage(from, buff, image, {quoted: mek, caption: ytt})
...
index.sendMessage(from, buff, image, {quoted: mek, caption: ytt3})
...
* Link:* ${items[i].url}\n\n`
                ...
                await index.sendMessage(from, thumb, image, {quoted: mek, caption: `${hehe}───────────────\n
`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdreply:{title:`SEARCH: ${args[0]}`,body:"YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/`}}})
            } catch(e) {
...
┃💌 : wa.me/6283115824395
┃💌 : ==============
┃💌 : Instagram : _*@index_store20*_
┃💌 : YouTube : _*index Zex's*_
┗━━⬣`
				sendButLocation(from, `${memek}`, `Subscribe Channel\nindex Zex!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'Done 🌌'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
case 'rdp':
case 'vps':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				memek =`index Store Promo only Rdp
OS All Windows!
...
case 'donate': 
       case 'donasi':
            gopeynya = 'https://e.top4top.io/p_21336v4de1'
            buff = await getBuffer(gopeynya)
            teksnya = `
DONASI OM\nSESUNGGUH NYA ORANG YANG MEMBERI LEBIH BAIK DARI MENERIMA ☝\n\nDana ️🎁: 083115824395\nOvo 🎊: Not Found\nPulsa Axis📺: 083115824395\nPulsa Tsel 📺: Not Found
`
            ...
            case 'exif':
				...
				case 'sticker':
					...
					if (!isRegistered) return reply(`Registrasi Dulu Kak Ketik .verify`)
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await index.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											index.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
...
case 'stats':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				var groups = index.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = index.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = index.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${index.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${index.user.phone.wa_version}
Os Version: ${index.user.phone.os_version}
`
reply(stamtus)
break		
				case 'return':
				if (!isOwner) return reply(mess.only.ownerB)
					return index.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return index.sendMessage(from, `root @index pembuat:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await index.downloadAndSaveMediaMessage(encmedia)
						...
						exec(`ffmpeg -i ${media} ${ran}`, (err) => {
							...
							index.sendMessage(from, buffer, image, {quoted: mek})
							fs.unlinkSync(ran)
						})
					}
					break
              ...
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				...
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await index.groupMetadata(wanu[0])
				...
				index.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return reply(`Registrasi dulu Kak ketik .verify`)
				runtime = process.uptime()
				...
				sendButLocation(from, `${memek}`, `Subscribe Channel\nGua!!`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}owner`,buttonText:{displayText:'AUTHOR🍁'},type:1}], {contextInfo: { mentionedJid: [ptod,dtod,otod,stod]}})
				break
			case 'setbio':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
				if (args.length < 1) return reply('Teksnya?')
					iyek = body.slice(8)
					index.setStatus(`${iyek}`)
...
const btngrass = {
    contentText: `${anu}`,
    footerText: '*Ervin Fahreza*',
    buttons: punten,
    headerType: 1
}
await index.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: mek})
					}
					break
					default:
		break
		 if (isMedia && isAuto && !mek.message.videoMessage || isQuotedImage) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								buffer = fs.readFileSync(ran)
								index.sendMessage(from, buffer, sticker, { quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
		   ...
		   if ((isMedia & isAuto && !mek.message.imageMessage || isQuotedVideo)) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								index.sendMessage(from, buff, sticker, { quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
